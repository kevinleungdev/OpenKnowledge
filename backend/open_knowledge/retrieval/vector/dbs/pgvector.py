...
class DocumentChunk(Base):
    ...
    if PGVECTOR_PGCRYPTO:
        ...
        vmetadata = Column(MutableDict.as_mutable(JSONB))
...
class PgvectorClient(VectorDBBase):
    ...
    def __init__(self):
        ...
        if not PGVECTOR_DB_URL:
            from open_knowledge.internal.db import Session
        ...
        try:
            ...
            # Use a conditional check to avoid permission issues
            ...
            # Create a tables if they do not exist
    ...
    def check_vector_length(self) -> None:
        ...
        try:
            ...
            # Table does not exists; no action needed
            return

        if "vector" in document_chunk_table.columns:
            vector_col = document_chunk_table.columns["vector"]
            vector_type = vector_col.type
            ...
            if isinstance(vector_type, Vector):
                ...
                raise Exception(
                    "The 'vector' column exists but is not of tyope 'Vector'."
                )
        else:
            raise Exception(
    ...
    def insert(self, collection_name: str, items: List[VectorItem]) -> None:
        try:
            if PGVECTOR_PGCRYPTO:
                ...
                self.session.commit()
                log.info(
                    f"Encrypted & inserted {len(items)} into '{collection_name}'")
                ...
                log.info(
                    f"Inserted {len(new_items)} into collection '{collection_name}'"
                )
        except Exception as e:
            self.session.rollback()
    ...
    def upsert(self, collection_name: str, items: List[VectorItem]) -> None:
        try:
            if PGVECTOR_PGCRYPTO:
                ...
                self.session.commit()
                log.info(
                    f"Encrypted & upserted {len(items)} into '{collection_name}'")
                ...
                self.session.commit()
                log.info(f"Upserted {len(items)} into '{collection_name}'")
        except Exception as e:
            self.session.rollback()
    ...
    def search(
        ...
        try:
            # Adjust query vectors to have length VECTOR_LENGTH
        ...
        try:
            if PGVECTOR_PGCRYPTO:
                ...
                for key, value in filter.items():
                    # decrpyt then check key: JSON filter after decryption
